	...
	function makeDraggable(target, panel){
		...
		panel.panel('panel').draggable({
			...
			proxy:function(source){
				var header_title_ = $(source).find(".panel-header");
				var panelHeader_ = $('<div class="panel-header" style="width: 100%;"></div>')
					.width(header_title_.width()-4)
					.append(header_title_.html());
				var portalPanel_ = $('<div class="panel" style="display: block;border:1px solid #99bbe8;"></div>')
					.width($(source).width()-4)
					.height($(source).height()-4)
					.append(panelHeader_);
				var p = $('<div class="portal-proxy"></div>').insertAfter(source);
				...
				$(this).hide();
				spacer = $('<div class="portal-spacer"></div>').html(portalPanel_).insertAfter(this);
